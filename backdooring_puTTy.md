...
The secondary objective was to gain practical experience with Adaptix C2 (i used Cobalt Strike in the past but i don't have 3K for a home license, so i have to find an alternative ahah) — specifically how to deliver and execute a beacon payload within a legitimate PE binary without relying on packers or off-the-shelf injectors. This meant solving real problems: the code cave being too small for the beacon, encrypting the payload on disk to avoid signature detection, writing a position-independent decryptor stub from scratch, and ensuring the host process continued to function normally after the payload executed.
...
Before writing a single byte, the following questions drove the entire 
analysis process:

- Where is the **Entry Point**? What is its RVA, and how do I convert 
  it to a file offset so I can read the actual bytes on disk?
- Where is the **`.text` section**? What are its `RawAddr`, `VirtualAddr`, 
  and `VirtualSize`?
- Is there a **code cave**? The difference between `RawSize` and `VirtualSize` 
  tells us how much zero-padding exists on disk — but is it enough to fit 
  a shellcode?
- Once I know the cave exists, **where exactly do I write**? The file offset 
  of the cave is `RawAddr + VirtualSize`. The RVA of the cave is 
  `VirtualAddr + VirtualSize`. Both are needed — one to write on disk, 
  one to calculate relative jumps.
- How do I **redirect execution** from the EP to the cave and back, without 
  breaking the original program flow?

Answering these questions systematically, using only PE-bear and `dd`, 
produced all the values needed to patch the binary by hand (after 2 hours of try and try and try and..).
```

Oh right, remember, E9 is the JMP instruction, it's necessary to insert other 4 bytes to redirect the jmp (x86 manual). 
...
| Target binary    | PuTTY last version (64-bit Windows)         |
...
Before modifying any binary, it is essential to understand how Windows executables are structured on disk and in memory (I did a specific project on this with two amazing resources).

### Structure Overview

What we need to understan for this project:
A Windows PE (Portable Executable) file is divided into regions called **sections**, each serving a specific purpose:

We will use PE-BEAR due to time constrains but every tool to dissasemble can be used ( yeah, i am trying to do this in one night actually, i am studying a lot).
...
**[+] IMPORTANT ->This padding is filled with null bytes and can be exploited as a code cave**.
...
(I have also used the analyzer i made only for fun, and honestly there are a lot of things i need to implement but this tool was created only to study a little better the PE :) )


![[Pasted image 20260307084603.png]]
...
EP_RVA = 0xBE504
### Section Table Extracted to facilitate some math 

| Field               | Meaning                                                                                        |
| ------------------- | ---------------------------------------------------------------------------------------------- |
| **Name**            | The name of the section (e.g. `.text`, `.data`), indicating its general purpose                |
| **Raw Address**     | **The offset in bytes from the start of the file where the section physically begins** on disk |
| **Raw Size**        | The actual **size in bytes of the section as stored on disk, padded to file alignment**        |
| **Virtual Address** | **The relative memory address (from image base)** where the section is loaded at **runtime**   |
| **Virtual Size**    | The actual size in bytes of the section **once loaded into memory, before alignment padding**  |
| **Characteristics** | Flags defining the section's permissions and properties (e.g. executable, readable, writable)  |
...
(skip=0xBD904 to avoid start on MZ bytes, jump directly on our interested bytes)

![[Pasted image 20260307090526.png]]

48 83 EC 28 → sub rsp, 28h (4 byte) 
E8 → CALL (1 byte) @ 0xBE508 
5B 02 00 00 → offset relative (4 byte) @ 0xBE509

Let me put under another light:
...
yes, it's important to build again everything in the cave, E8 is a call (48 83 EC 28 E8 5B 02 00 00).
...
Before injecting a real C2 beacon, I used a simple MessageBox shellcode
to verify the technique works end-to-end. The idea was simple: redirect
execution from the Entry Point to a code cave, run the shellcode, then
return to PuTTY as if nothing happened.
...
msfvenom reported `Payload size: 303 bytes` — but the actual file size
was **299 bytes**. I only discovered this later, after a wrong JMP back
offset. Always verify with `ls -la`.

299 bytes < 474 bytes available in the cave → fits. 

### Understanding What We Are About to Destroy

Before writing anything (or, as me, after destroying something), analyze the Entry Point bytes with Ghidra
(`G` → `1400BE504`):
```
...
The remaining `5B 02 00 00` at `0xBE509` become orphaned — they are
no longer a valid CALL without `E8`. This is why I had to save all
**9 bytes** and reconstruct them inside the cave.

The CALL target was determined from its relative offset:
```bash
dd if=putty.exe bs=1 skip=$((16#BD909)) count=4 2>/dev/null | xxd
# Result: 5B 02 00 00 → 0x25B (little-endian)
```
```
CALL target = 0xBE508 + 5 + 0x25B = 0xBE768
```

This offset `0x25B` is relative to position `0xBE508` — it will not
work if copied directly into the cave. It must be recalculated.

### Calculating the JMP to Cave

The JMP opcode `E9` takes a 4-byte relative offset (x86 manual):
```
...
CAVE_FILE_OFFSET (0xEC026):
  ...
  E9 xx xx xx xx        ← JMP back → 0xBE50D  (EP+9, first intact instruction)
```

### Recalculating the CALL Offset

Since the CALL is now at a different position inside the cave, its
relative offset must be recalculated:
...
After the CALL, a final JMP returns execution to `EP+9` — the first
instruction not overwritten by our patch (`ADD RSP, 0x28`):
```python
...
msfvenom reported 303 bytes, but `msgbox.bin` was actually 299 bytes.
My first JMP back offset was calculated on 303, landing 4 bytes past
the actual end of the shellcode — straight into garbage. Verified the
real size with `ls -la` and recalculated everything from `len(shellcode)`.

**2. E8 destroyed at EP — CALL not reconstructed**

The first version of the patcher only saved 5 bytes and only wrote
`48 83 EC 28` back into the cave. `E8` was gone and `5B 02 00 00` were
orphaned. PuTTY crashed immediately on return. Fixed by saving 9 bytes
and reconstructing the CALL with a calculated relative offset.
...
After clicking OK on the MessageBox, the process died before the JMP
back could execute. I inserted a JMP short (`EB 09`) at offset `0x120`
— immediately after the MessageBox call — to skip the entire
ExitProcess block.

**4. RSP corruption**

Even after fixing ExitProcess, PuTTY still crashed on return.
msfvenom's shellcode modifies **RSP internally** — it aligns it, allocates
space, and never fully restores it. When my JMP back executed, RSP
pointed nowhere valid and `ADD RSP, 0x28` at EP+9 caused an access
violation. Fixed by wrapping the shellcode with:
...
After patching, I imported `putty_patched.exe` and navigated to
`1400ECC26` to verify the cave layout:
...
The tool i initially built (not the one above, which is the correct one), failed to patch correctly for a simple reason: it was built to **read** PE files, not **write** them. It had no logic to handle file alignment, no way to recalculate relative offsets after moving bytes around, and no concept of the difference between modifying bytes on disk vs. what the loader maps into memory after msfvenom will entry in action.

![[Pasted image 20260307101830.png]]

### Issues Patched, rsp restored and a correct jump

![[Pasted image 20260308100625.png]]

### Final Result 

The MessageBox appeared on launch. After clicking OK, PuTTY opened and functioned normally, confirming the full execution chain worked end-to-end.

![[Pasted image 20260307132518.png]]
---
...
A new section named `.laz` is appended to the PE after `.reloc`. It requires `Execute + Read + Write` permissions (`0xE0000020`) because it will hold shellcode that must be decrypted and executed at runtime.
...
The new section must start after the last existing section, aligned to `SectionAlignment` (0x1000) for the virtual address and `FileAlignment` (0x200) for the raw offset on disk:
...
Unlike Phase 1, there is no JMP back here — the beacon takes over the main thread and never returns. This is a known limitation, discussed in the conclusions.

### Why XOR Encryption?

Writing the raw Adaptix beacon into the PE file triggers immediate detection by Windows Defender on disk — the shellcode bytes are well-known signatures.
...
The stub is 32 bytes of position-independent x64 shellcode. It must work regardless of where it is loaded in memory — no hardcoded addresses. The `CALL $+5` / `POP RAX` trick is the standard PIC technique for self-location:
...
Note: the stub uses `PUSH RAX` / `POP RAX` to save the beacon address around the XOR loop. Since the loop decrypts memory in-place starting immediately after the stub, in theory the stack could be overwritten during decryption. In practice the beacon executed correctly regardless. The clean fix is to use `MOV RDI, RAX` / `JMP RDI` to avoid touching the stack entirely.
...
The correct fix is to delay or redirect execution so that PuTTY fully initializes before the beacon runs. Two approaches I identified and partially explored:

**Option A — CreateThread**: launch the beacon in a separate thread from the cave, then JMP back to EP+9 as in Phase 1. The main thread continues normally and PuTTY opens. This requires resolving `CreateThread` from the IAT and building a stub that sets up the correct x64 calling convention (RCX, RDX, R8, R9, shadow space).
...
![[Pasted image 20260307121639.png]]

The new `.laz` section is visible in PE-bear with:
...
The generated `agent.x64.bin` (96,255 bytes) was saved to the `Payloads/` directory and patched.

![[Pasted image 20260307120345.png]]
...
![[Pasted image 20260307120618.png]]

### Verification in x64dbg

The WinDbg debugger confirmed:

- The patched EP at `0xBE504` contained `E9 F7 6A 0E 00` (JMP to `.laz` section at `putty+0x1A5000`)
...
- After decryption, valid x64 instructions were visible at `putty+0x1A5020` (`push rsi`, `mov rsi, rsp`, `and rsp, 0FFFFFFFFFFFFFFF0h`), confirming successful in-memory decryption and execution

The following issues were encountered and resolved during development:

- **JMP back offset miscalculation** — the initial offset was calculated assuming 303 bytes of shellcode, but `msgbox.bin` was 299 bytes, causing the JMP back to land at the wrong address

- **Stack corruption in decryptor stub** — the stub used `PUSH RAX` to save the beacon pointer before the XOR loop. Since the loop decrypts memory in-place starting immediately after the stub, the pushed value on the stack was potentially corrupted during decryption. The recommended fix is to replace `PUSH RAX` / `POP RAX` with `MOV RDI, RAX` / `JMP RDI` to avoid touching the stack entirely. This did not affect execution in practice as the beacon launched successfully regardless.
...
*This research was conducted for educational purposes. All techniques described are documented in public threat intelligence reports. No systems were harmed outside of an isolated lab environment. Only my sleep time was involved.*
